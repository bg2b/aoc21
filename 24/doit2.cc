...
#ifdef HAVE_ALU
string search() {
  string model_num(14, '1');
  while (true) {
    int wrong = alu(model_num.c_str());
    if (wrong == -1)
      return model_num;
    assert(wrong >= 0 && wrong < int(model_num.size()));
    // Everything to the right of wrong should reset to 1
    for (unsigned reset = wrong + 1; reset < model_num.size(); ++reset)
      model_num[reset] = '1';
    while (wrong >= 0 && model_num[wrong] == '9')
      // Carry
      model_num[wrong--] = '1';
    // Should not have carried off the end
    assert(wrong >= 0 && model_num[wrong] < '9');
    ++model_num[wrong];
  }
}
#endif
...
#ifdef HAVE_ALU
  printf("Min valid model number %s\n", search().c_str());
#else
  compile(cin);
#endif
  return 0;
